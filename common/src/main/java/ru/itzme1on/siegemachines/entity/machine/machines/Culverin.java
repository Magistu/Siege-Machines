...
{
    public static final Supplier<EntityType<Culverin>> TYPE = Suppliers.memoize(() -> EntityType.Builder.create(Culverin::new, SpawnGroup.MISC)
    ...
    public Culverin(EntityType<? extends MobEntity> entityType, World world) {
        super(entityType, world, MachineType.CULVERIN);
    }
    ...
    public void registerControllers(AnimationData data)
    {
        AnimationController<?> wheels_controller = new AnimationController<>(this, "wheels_controller", 1, (t) -> {
            ...
            return wheelsSpeed += 0.013 * this.wheelsSpeed;
...
//                    net.minecraftforge.event.ForgeEventFactory.onPlayerDestroyItem(player, this.useItem, hand);
    ...
    public void tick() {
        ...
        if (!world.isClient && this.isOnGround()) this.setVelocity(this.getWheelsDeltaMovement());
    ...
    public double getWheelsSpeed() {
        if (this.isOnGround())
            return this.getRotationVec(5.0f).multiply(1, 0, 1).dotProduct(this.getVelocity());
