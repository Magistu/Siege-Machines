...
public class ClientProxyForge {
    ...
    public static void setup(IEventBus modEventBus) {
        ...
        modEventBus.addListener(ClientProxyForge::registerEntityRenderers);
        modEventBus.addListener(ClientProxyForge::registerItemRenderers);
    ...
    public static void onKeyPressedEvent(InputEvent.Key ev) {
        ...
        if (Minecraft.getInstance().options.keyJump.isDown() && player.getVehicle() instanceof SiegeLadder) {
            ModNetwork.sendToServer(new C2SPacketMachineUse());
        }
    ...
    public static void registerOverlay(RegisterGuiLayersEvent event) {
        event.registerAbove(VanillaGuiLayers.CROSSHAIR, SiegeMachines.id("crosshair"), ClientProxyForge::renderCrosshair);
    }
