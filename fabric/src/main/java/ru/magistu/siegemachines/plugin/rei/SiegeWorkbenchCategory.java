...
public class SiegeWorkbenchCategory implements DisplayCategory<SiegeWorkbenchRecipeDisplay> {
    ...
    public @NotNull List<Widget> setupDisplay(SiegeWorkbenchRecipeDisplay display, Rectangle bounds) {
        ...
        for (int y = 0; y < display.getRecipe().getHeight(); ++y) {
            for (int x = 0; x < display.getRecipe().getWidth(); ++x) {
                int index = y * display.getRecipe().getWidth() + x;
        ...
        widgets.add(Widgets.createArrow(new Point(startX + 18 * display.getRecipe().getWidth() + 5, startY + 18)));

        widgets.add(Widgets.createSlot(new Point(startX + 18 * display.getRecipe().getWidth() + 5 + 30, startY + 18)).entries(display.getOutputEntries().get(0)).markOutput());
