...
public abstract class Missile extends ThrowableItemProjectile implements Explosive {
    ...
    public Missile(EntityType<? extends Missile> entitytype, Level level, Vector3d pos, LivingEntity shooter, Entity engine, MissileType type) {
        super(entitytype, shooter.getControllingPassenger() != null ? shooter.getControllingPassenger() : shooter, level);
        this.type = type;
        this.engine = engine;
    ...
    protected void handleEntityHit(EntityHitResult entityHit) {
        ...
        float damage = (float) (this.type.specs.mass.get() * this.getDeltaMovement().length() * 0.1);
        ...
        if (!this.level().isClientSide() && this.type.specs.explosive.get()) {
            this.explode(pos.subtract(this.getDeltaMovement().normalize().scale(0.5)), BlockInteraction.DESTROY);
        }
    ...
    private void applyKnockback(Entity entity) {
        ...
        if (entity instanceof LivingEntity livingentity) {
            knockback = ProtectionEnchantment.getExplosionKnockbackAfterDampener(livingentity, this.type.specs.knockback.get());
        } else {
            knockback = this.type.specs.knockback.get();
        }
    ...
    protected void handleBlockHit(BlockHitResult blockHit) {
        ...
        if (!this.level().isClientSide() && this.type.specs.explosive.get()) {
            BlockInteraction interaction = getExplosionBlockInteraction();
    ...
    public Explosion explode(Vec3 pos, BlockInteraction mode) {
        ...
        MissileExplosion explosion = new MissileExplosion(level(), source, damageSources().explosion(this, indirectSource), explosionDamageCalculator, x, y, z, size, fired, mode);
    ...
    public void handleEntityEvent(byte id) {
        if (id == MISSILE_EXPLOSION && this.level().isClientSide()) {
            this.explode(this.position(), BlockInteraction.KEEP);
        } else {
